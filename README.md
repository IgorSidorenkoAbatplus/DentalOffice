...
Краткая инструкция использования файлов репозитория:
	для успешного запуска и использования приложения, требуется: OS Windows 7 или выше, предустановленная версия Microsoft SQL Server 2014 Express или выше (доступно для скачивания https://www.microsoft.com/ru-RU/download/details.aspx?id=42299);
	скачать доступные для этого файлы данного репозитория и поместить их в папку DentalOffice (место расположения папки значения не имеет);
	назначить полные права доступа к содержащимся в папке DentalOffice файлам БД (файлу базы данных и файлу лога базы данных);
	присоединить базу данных к серверу;
	запустить файл приложения DentalOffice.exe
	использовать при первом входе в приложение логин: Sidorenko пароль: 3829783;
	в дальнейшем, рекомендуется создать и использовать для доступа в приложение собственную учетную запись специалиста, с собственным логином и паролем и полной ролью доступа, и удалить текущую учетную запись Sidorenko.
